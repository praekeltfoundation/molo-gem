...
def add_reaction_questions(self, main_lang, child_languages, question_index,
                           reaction_questions, question_choices):
    for reaction_question in reaction_questions:
        if reaction_questions.get(reaction_question).get(main_lang.locale):
            main_reaction_question = create_reaction_question(
                reaction_questions.get(
                    reaction_question).get(main_lang.locale), question_index)
            add_reaction_choices(
            ...
            for child_lang in child_languages:
                if reaction_questions.get(
                    ...
                    create_reaction_question_translation(
...
def create_reaction_question(title, question_index):
...
def create_reaction_question_translation(
        main_reaction_question, language, trans_title, question_index):
    if not ReactionQuestion.objects.filter(
            title=trans_title).child_of(question_index).exists():
        translated_reaction_question = create_reaction_question(
            trans_title, question_index)
        if translated_reaction_question:
            language_relation = translated_reaction_question.languages.first()
...
def add_reaction_choices(
    ...
    for reaction_choice in reaction_choices:
        if reaction_choices.get(reaction_choice).get(main_lang.locale):
            main_reaction_choice = create_reaction_choice(
                reaction_choices.get(reaction_choice).get(main_lang.locale),
                question, main_lang.locale)
            create_reaction_choice_image(main_reaction_choice)

            for child_lang in child_languages:
                if reaction_choices.get(
                        reaction_choice).get(child_lang.locale):
                    create_reaction_choice_translation(
...
def create_reaction_choice(title, question, language):
...
def create_reaction_choice_translation(
        main_reaction_choice, language, trans_title, question):
    if not ReactionQuestionChoice.objects.filter(
            title=trans_title, languages__language__locale=language).child_of(
            question).exists():
...
def create_reaction_choice_image(main_reaction_choice):
