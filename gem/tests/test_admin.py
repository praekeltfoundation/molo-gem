...
class TestCommentReportingModelAdmin(TestCase, GemTestCaseMixin):
    ...
    def test_admin_api(self):
        response = self.client.get('/admin/api/v2beta/pages/?child_of=1&for_explorer=1')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.data)

    def test_admin_api_with_invalid_input(self):
        response = self.client.get('/admin/api/v2beta/pages/?child_of=1&for_explorer=1&fields=121223')
